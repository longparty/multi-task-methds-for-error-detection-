...
class BiLSTM(nn.Module):
    ...
    def __init__(self, config, word_embeddings, embedding_dim, hidden_dim, vocab_size, tagset_size,tagset_size2, tagset_size3, tagset_size4):
        ...
        self.gate2 = nn.Linear(embedding_dim, 3)
        self.gate3 = nn.Linear(embedding_dim, 3)
        self.gate4 = nn.Linear(embedding_dim, 3)
    ...
    def forward(self, sentence):
        ...
        gate_out2 = self.gate2(embeds)
        gate_out2 = torch.FloatTensor(gate_out2)
        gate_wight2 = gate_out2.mean(0)
        Softmax = nn.Softmax(dim=0)
        gate_wight2 = Softmax(gate_wight2)

        gate_out3 = self.gate3(embeds)
        gate_out3 = torch.FloatTensor(gate_out3)
        gate_wight3 = gate_out3.mean(0)
        Softmax = nn.Softmax(dim=0)
        gate_wight3 = Softmax(gate_wight3)

        gate_out4 = self.gate4(embeds)
        gate_out4 = torch.FloatTensor(gate_out4)
        gate_wight4 = gate_out4.mean(0)
        Softmax = nn.Softmax(dim=0)
        gate_wight4 = Softmax(gate_wight4)

        lstm_out_t1 = lstm_out1 * gate_wight[0] + lstm_out2 * gate_wight[1] + lstm_out3 * gate_wight[2]
        lstm_out_t2 = lstm_out1 * gate_wight2[0] + lstm_out2 * gate_wight2[1] + lstm_out3 * gate_wight2[2]
        lstm_out_t3 = lstm_out1 * gate_wight3[0] + lstm_out2 * gate_wight3[1] + lstm_out3 * gate_wight3[2]
        lstm_out_t4 = lstm_out1 * gate_wight4[0] + lstm_out2 * gate_wight4[1] + lstm_out3 * gate_wight4[2]

        tag_space = self.hidden2tag(lstm_out_t1.view(len(sentence), -1))
        tag_scores = F.log_softmax(tag_space, dim=1)

        tag_space2 = self.hidden2tag2(lstm_out_t2.view(len(sentence), -1))
        tag_scores2 = F.log_softmax(tag_space2, dim=1)

        tag_space3 = self.hidden2tag3(lstm_out_t3.view(len(sentence), -1))
        tag_scores3 = F.log_softmax(tag_space3, dim=1)

        tag_space4 = self.hidden2tag4(lstm_out_t4.view(len(sentence), -1))
